...
class Center(object):
    def __init__(self, fix_seed, env, x, y, routRule, AGV_num, WS_num, AGV_disRuleV, AGV_disRuleW, Ledispatch = "None", Task = ["None", "None"]):
        if fix_seed != None:
    ...
    def Job_arrive(self):
        while True:
            inter_arrival = np.random.exponential(self.job_arrival_rate)
...
#                print(self.Time, ': go ')
                if hasattr(self, "LEV") and self.Task[0] != "Collect":
                    ...
                    # self.status.currentDegree[0] = (self.status.SL_mean) / (self.status.ROT_mean * self.Jnum)
                ...
                if hasattr(self, "LEW") and self.Task[1] != "Collect":
                    ...
                    # self.status.currentDegree[1] = (self.status.SL_mean) / (self.status.ROT_mean * self.Jnum)
...
#                    print(self.Time, ': Nice')
                    s = self.status.currentstate[0]
                    # degree = self.status.currentDegree[0]

                    s_ = self.status.Get("V")
                    # degree_ = (self.status.SL_mean) / (self.status.ROT_mean * self.Jnum)

                    # r = -self.status.meantardiness
                    #################################################################
                    r = (self.status.SL_mean) / (self.status.ROT_mean * self.Jnum)
                    #################################################################
                    # if degree_ > degree:
                    #     r = 1
                    # else:
...
#                        print("looks good")
                        ...
                        if self.tempV == 6:
                            pass
                            # print(self.Time, " --  Yeah~~")
                            # s_ = self.status.Get("v")
                            # print('learned')
                            # self.LEV.Learning(s, a, s_, r)
                        else:
                            #r = -round(self.MeanTardiness/10, 0)
                            #r = 2.5-a
                            #################################################################
                            self.LEV.store_transition(s, a, s_, r)
                            #################################################################
                            # self.LEV.store_temp(s, a, s_)
                            #################################################################
...
#                    print(self.Time, ': Nice--- ')
                    ...
                    # degree = self.status.currentDegree[1]
...
#                        print("- looks good - ")############################
            ...
            if round(self.Time - self.Warmup) % 500 == 0:
                ...
                self.STDTardiness = np.std(tard)
                self.status.meantardiness = np.mean(tard1)

            # #################################################################
            # if round(self.Time) == 20000:
            #     if hasattr(self, "LEV"):
            #         future_step = 0
            #         for i in self.LEV.tempstore:
            #             s = i[0]
            #             a = i[1]
            #             s_ = i[2]
            #             r = -self.MeanTardiness #* (self.R_disc ** future_step)
            #             self.LEV.store_transition(s, a, s_, r)
            #         self.LEV.tempstore = []
            #
            #     if hasattr(self, "LEW"):
            #         future_step = 0
            #         for i in self.LEW.tempstore:
            #             s = i[0]
            #             a = i[1]
            #             s_ = i[2]
            #             r = -self.MeanTardiness #* (self.R_disc ** future_step)
            ...
            if self.LEV.wantLearn == True and self.LEW.wantLearn == True:
                if round(self.Time) > self.Period and round(self.Time) % self.Period == 0:
                    if hasattr(self, "LEV"):
                        self.LEV.Learning()
                    elif hasattr(self, "LEW"):
                        self.LEW.Learning()
